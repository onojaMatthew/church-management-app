...
import { churchResetPassword } from "../../../store/actions/actions_church";
...
const ResetPassword = () => {
  ...
  const handleSubmit = (e) => {
    ...
    dispatch(churchResetPassword(data));
  }

  useEffect(() => {
    console.log(window.location.pathname)
    const tken = window.location.pathname.slice(23)
    setToken(tken)
  }, [])
      ...
      {done ? (
        <div>
          ...
          <p className="text-center" style={{
            ...
            cursor: "pointer",
          }} onClick={() => history.push("/church-login")}>here</span> to login</p>
        </div>
      ) : (
        <Row>
